...
If using Codespaces there is a problem with an outdated sqlite3 in the image
We need to
pip install pysqlite3-binary
Then place the following lines in "mmemgpt.agent_store.chroma.py" before the import chromadb line
**import**('pysqlite3')
import sys
...
You can also use MemGPT to depoy agents as a _service_. You can use a MemGPT server to run a multi-user, multi-agent application on top of supported LLM providers.
